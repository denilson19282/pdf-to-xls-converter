...
class Inter1FaturaXLSConverter(PDFXLSConverterInterface):
    ...
    def _find_date(self, text):
        return re.search(r"\b\d{1,2} [a-z]{3} \d{4}\b", text)

    def _find_parcela(self, text):
        return re.search(r'\(Parcela (\d+) De (\d+)\)', text)
    
    def _find_unwanted(self, text):
        return text in ['Pagamento On Line', 'Pagamento De Fatura']

    def _convert_date(self, text):
        day, month, year = re.search(r"(\d{1,2}) ([a-z]{3}) (\d{4})", text).groups()
